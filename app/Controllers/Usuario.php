...
class Usuario extends BaseController
{
    ...
    public function applyInstitutionalUserEditPayload(array $payload, array $actorContext, int $idSesionUsuario, string $scriptName = 'Usuario.applyInstitutionalUserEditPayload')
    {
        ...
        if ($idUsuario <= 0) {
            return $this->respond([
                'error' => true,
                'respuesta' => 'La solicitud no contiene el usuario a editar.',
            ]);
        }
        ...
        if (!$this->resolver->canMutateRow($actorContext, $usuarioActual)) {
            return $this->respond([
                'error' => true,
                'respuesta' => 'No tienes permisos para aplicar esta edición.',
            ]);
        }

        return $this->handleInstitutionalUserEdit($payload, $usuarioActual, $actorContext, $idSesionUsuario, $scriptName, true);
    }
    ...
    private function buildInstitutionalAssignment(string $grupo, int $perfilGrupo): array
    {
        ...
        if ($grupo === 'fic') {
            $assignment['id_fic_perfil'] = $perfilGrupo;
        } elseif ($grupo === 'ug') {
            $assignment['id_ug_perfil'] = $perfilGrupo;
        } elseif ($grupo === 'secul') {
            $assignment['id_secul_perfil'] = $perfilGrupo;
        }
