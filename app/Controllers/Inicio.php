   ...
   public function Hospedaje()
{
    ...
    public function getCambiosRealtime()
    {
        $session = \Config\Services::session();
        $resolver = new UsuarioPerfilResolver();
        $contextoUsuario = $resolver->resolve($session->get());
        $usuarioDashboard = $this->resolveSecturiDashboardUsuario();
        $puedeConsultarGlobal = !empty($usuarioDashboard)
            || !empty($contextoUsuario['can_access_secturi_dashboard'])
            || !empty($contextoUsuario['is_ti_master'])
            || !empty($contextoUsuario['can_access_user_catalog'])
            || !empty($contextoUsuario['is_provider_flow'])
            || !empty($contextoUsuario['is_recepcion_flow'])
            || (int) ($session->get('id_establecimiento') ?? 0) > 0;

        if (!$puedeConsultarGlobal) {
            return $this->response->setStatusCode(403)->setJSON([
                'ok' => false,
                'message' => 'No tienes permisos para consultar cambios.',
            ]);
        }

        $scopeParam = trim((string) ($this->request->getGet('scopes') ?? ''));
        $scopes = array_values(array_filter(array_unique(array_map(static function ($scope): string {
            return strtolower(trim((string) $scope));
        }, explode(',', $scopeParam)))));

        $allowedScopes = ['cajero', 'solicitudes_folio', 'solicitudes_operativo', 'documental'];
        $scopes = array_values(array_intersect($scopes, $allowedScopes));
        if (empty($scopes)) {
            $scopes = ['cajero'];
        }

        $versionsRaw = (string) ($this->request->getGet('versions') ?? '{}');
        $versions = json_decode($versionsRaw, true);
        $versions = is_array($versions) ? $versions : [];

        $currentVersions = [];
        $events = [];
        foreach ($scopes as $scope) {
            $version = $this->getRealtimeScopeVersion($scope);
            $currentVersions[$scope] = $version;
            $previous = isset($versions[$scope]) ? (int) $versions[$scope] : 0;

            if ($previous > 0 && $version > $previous) {
                $events = array_merge($events, $this->getRealtimeScopeEvents($scope, $version));
            }
        }

        return $this->response->setJSON([
            'ok' => true,
            'has_changes' => !empty($events),
    ...
    private function getRealtimeScopeVersion(string $scope): int
    {
        $db = \Config\Database::connect();
        try {
            if ($scope === 'cajero') {
                return $this->getRealtimeMaxTimestamp($db, 'usuario', ['fec_act', 'fec_reg']);
            }

            if ($scope === 'solicitudes_folio') {
                return $this->getRealtimeMaxTimestamp($db, 'solicitud_usuario', ['fec_act', 'fec_reg'], [
                    'tipo_solicitud' => ['alta_usuario_fic', 'alta_usuario_secul', 'alta_usuario_ug'],
                ]);
            }

            if ($scope === 'solicitudes_operativo') {
                return $this->getRealtimeMaxTimestamp($db, 'solicitud_usuario', ['fec_act', 'fec_reg'], [
                    'tipo_solicitud' => ['alta_gerente', 'alta_recepcion'],
                ]);
            }

            if ($scope === 'documental') {
                return $this->getRealtimeMaxTimestamp($db, 'facturas', ['fec_act', 'fec_reg']);
            }
        } catch (\Throwable $e) {
            log_message('warning', 'Inicio.getRealtimeScopeVersion[' . $scope . ']: ' . $e->getMessage());
        }

        return 0;
    }

    private function getRealtimeMaxTimestamp($db, string $table, array $dateFields, array $filters = []): int
    {
        if (!$db->tableExists($table)) {
            return 0;
        }

        $existingFields = array_values(array_filter($dateFields, static function (string $field) use ($db, $table): bool {
        ...
        $fieldExpressions = array_map(static function (string $field): string {
            return 'UNIX_TIMESTAMP(MAX(' . $field . ')) AS ts_' . $field;
        }, $existingFields);

        $builder = $db->table($table)->select(implode(', ', $fieldExpressions), false);
        if ($db->fieldExists('visible', $table)) {
            $builder->where('visible', 1);
        }

        foreach ($filters as $field => $value) {
            if (!$db->fieldExists((string) $field, $table)) {
                continue;
            }

            if (is_array($value)) {
                $builder->whereIn((string) $field, $value);
            } else {
                $builder->where((string) $field, $value);
            }
        }
        ...
        foreach ($existingFields as $field) {
            $timestamps[] = (int) ($row['ts_' . $field] ?? 0);
        }

        return max($timestamps ?: [0]);
    }

    private function getRealtimeScopeEvents(string $scope, int $version): array
    {
        if ($scope === 'cajero') {
            return [
                [
                    'name' => 'fic:cajero-usuarios-actualizados',
        ...
        if ($scope === 'solicitudes_folio') {
            return [
                [
                    'name' => 'fic:solicitud-folio-actualizada',
                    'detail' => ['scope' => $scope, 'version' => $version],
                ],
            ];
        }
        ...
        if ($scope === 'solicitudes_operativo') {
            return [
                [
                    'name' => 'fic:solicitud-operativa-actualizada',
                    'detail' => ['scope' => $scope, 'version' => $version],
                ],
            ];
        }
        ...
        if ($scope === 'documental') {
            return [
                [
                    'name' => 'fic:estado-documental-actualizado',
                    'detail' => ['scope' => $scope, 'version' => $version],
                ],
            ];
        }
