...
<div class="container-fluid py-4"
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <div>
            <h3 class="mb-1 text-white"><?= $cajeroSoloConsulta ? 'Consulta de usuarios y folios' : 'AdministraciÃƒÂ³n de cajeros' ?></h3>
            <p class="text-muted mb-0"><?= $cajeroSoloConsulta ? 'Consulta llegadas, documentos y ÃƒÂ³rdenes sin modificar usuarios.' : 'Consulta, registra, edita o elimina cajeros.' ?></p>
        </div>
        <div class="d-flex flex-wrap gap-2 justify-content-end">
            <?php if ($cajeroAccesoTiInicio): ?>
            <a href="<?= esc($cajeroRegresarUrl, 'attr') ?>" class="btn btn-outline-secondary">
                <i class="mdi mdi-arrow-left me-1"></i> AtrÃƒÂ¡s
            </a>
            <?php endif; ?>
    ...
    <?php if ($cajeroSoloConsulta): ?>
        <div class="alert alert-info" role="status">Modo consulta: las acciones de carga, activaciÃƒÂ³n y eliminaciÃƒÂ³n estÃƒÂ¡n deshabilitadas para este perfil.</div>
    <?php endif; ?>
        ...
        <div class="card-body">
            <div class="row g-2 align-items-end mb-3">
                <div class="col-12 col-md-4 col-lg-3">
                    <label class="form-label" for="filtro_dia_llegada">DÃƒÂ­a de llegada</label>
                    <input type="date" class="form-control" id="filtro_dia_llegada">
                </div>
                <div class="col-12 col-md-auto">
                    <button type="button" class="btn btn-outline-light" id="limpiar_filtro_dia_llegada" disabled>Todos los dÃƒÂ­as</button>
                </div>
                <div class="col-12 col-md">
                    <div class="text-muted small" id="filtro_dia_llegada_estado">Mostrando todos los folios por dÃƒÂ­a de llegada.</div>
                </div>
            </div>
            ...
            <form id="cajeroForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label" for="contrasenia">ContraseÃƒÂ±a</label>
                            <input type="password" class="form-control" name="contrasenia" id="contrasenia">
                            <small class="text-muted">En ediciÃƒÂ³n, dÃƒÂ©jala vacÃƒÂ­a para conservar la actual.</small>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content bg-dark text-white">
            ...
            <div class="modal-footer border-secondary">
                <a href="#" id="cajeroDocumentosOpen" class="btn btn-outline-info disabled" target="_blank" rel="noopener">Abrir en nueva pestaÃƒÂ±a</a>
...
window.cajeros = Object.assign(window.cajeros || {}, {
    ...
    iniciar() {
        if (typeof $.fn.bootstrapTable !== 'function') {
            console.error('Bootstrap Table no estÃƒÂ¡ disponible.');
        ...
        $('#cajerosTable').bootstrapTable({
            ...
            responseHandler: (response) => {
                ...
                if (Array.isArray(response)) {
                    ...
                    console.error('Respuesta invÃƒÂ¡lida al cargar cajeros:', response);
                    return [];
                }
    ...
    estado(value) {
        if (Number(value) === 1) return '<span class="badge bg-success">SÃƒÂ­</span>';
    ...
    descargarXlsx() {
        ...
        if (!baseUrl) {
            Swal.fire('AtenciÃƒÂ³n', 'No fue posible resolver la ruta de descarga.', 'warning');
            return;
        }
    ...
    abrirDocumento(url) {
        ...
        if (!documentoUrl) {
            Swal.fire('AtenciÃƒÂ³n', 'No hay documento disponible.', 'warning');
            return;
        }
    ...
    verDocumentos(idUsuario) {
        ...
        if (!row) {
            Swal.fire('AtenciÃƒÂ³n', 'No fue posible resolver los documentos del usuario.', 'warning');
            return;
        }
    ...
    abrirDocumento(field) {
        ...
        if (!doc || doc.path === '') {
            Swal.fire('AtenciÃƒÂ³n', 'No hay archivo disponible.', 'warning');
            return;
        }
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg'].indexOf(ext) !== -1) {
            ...
            preview = '<div class="alert alert-info m-3">No hay vista previa para este tipo de archivo. Usa el botÃƒÂ³n para abrirlo en una nueva pestaÃƒÂ±a.</div>';
        }
    ...
    activarQr(idUsuario) {
        ...
        Swal.fire({
            ...
            text: 'Se marcarÃƒÂ¡ el QR del usuario como activo.',
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'SÃƒÂ­, activar',
            cancelButtonText: 'Cancelar'
        }).then((result) => {
    ...
    rechazarActivacionQr(idUsuario) {
        ...
        Swal.fire({
            title: 'Ã‚Â¿Rechazar activaciÃƒÂ³n QR?',
            text: 'Se retirarÃƒÂ¡ la activaciÃƒÂ³n y el usuario podrÃƒÂ¡ iniciar nuevamente su proceso.',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'SÃƒÂ­, rechazar',
            cancelButtonText: 'Cancelar'
        }).then((result) => {
            if (!result.isConfirmed) return;
            ...
            $.ajax({
                ...
                if (!response || response.success !== true) {
                    Swal.fire('AtenciÃƒÂ³n', response && response.message ? response.message : 'No fue posible rechazar la activaciÃƒÂ³n del QR.', 'warning');
                    return;
                }
                ...
                const response = request.responseJSON || {};
                Swal.fire('Error', response.message || 'No fue posible rechazar la activaciÃƒÂ³n del QR.', 'error');
            });
        });
    },
    ...
    guardar() {
        ...
        $.ajax({
            ...
            if (response.error) {
                Swal.fire('AtenciÃƒÂ³n', response.respuesta, 'warning');
                return;
            }
    ...
    eliminar(idUsuario) {
        Swal.fire({
            ...
            text: 'El registro dejarÃƒÂ¡ de mostrarse en la tabla.',
            icon: 'warning',
            showCancelButton: true,
